...
# actual naem of disease and symtoms from label incoder 
...
def home():
    if request.method == 'POST':
        ...
        # Initialize user_symptoms with an empty list
        user_symptoms = []

        # Check if symptoms are empty or contain invalid symptoms
        if not symptoms or not all(symptom.strip("[]' ") in symptoms_dict for symptom in user_symptoms):
            ...
            # Check if all user symptoms are in the symptoms_dict
            if not all(symptom in symptoms_dict for symptom in user_symptoms):
                message = "One or more symptoms are misspelled or invalid"
                return render_template('index.html', message=message)
